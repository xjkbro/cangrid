...
// Reverses buildPublicUrl — recovers the object key from a stored public
// URL so a deleted Image record's file can also be removed from the
// bucket. Returns null if the URL doesn't look like one of ours (e.g. it
// predates this storage backend), so callers can skip cleanup safely.
export function extractObjectKey(publicUrl) {
    const prefix = `${getMinioPublicBaseUrl()}/${MINIO_BUCKET()}/`;
